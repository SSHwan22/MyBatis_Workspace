...
public class BoardDao {
	...
	public ArrayList<Board> selectList(SqlSession sqlSession, PageInfo pi){
		...
		// * offset : 몇 개의 게시글을 건너뛰고 조회하걸건지에 대한 값을 설정할 수 있음.
		/*
		 * ex) boardLimit가 5일경우
		...
		// RowBounds객체를 넘겨줘야하는 경우, selectList함수의 오버로딩된 매소드 중 매개변수가 3개인 메소드를 사용해야 함.
		// 딱히 두번째 자리에 파라미터로 넘길 값이 없더라도 null값이라도 넘겨줘야한다.
		
		// 조회할 데이터가 얼마 없을 떄 사용하는게 유용하다. 많은 데이터가 있다면 쿼리문으로 제어하는게 좋을거다.
